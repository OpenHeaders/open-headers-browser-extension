...
interface PreNavRecordingResult {
  success: boolean;
  tabId: number;
  recordId: string;
}

interface PreNavResponse {
  success?: boolean;
  error?: string;
}

export async function startRecordingNewTab(url: string): Promise<PreNavRecordingResult> {
...
export async function startRecordingCurrentTabWithReload(): Promise<PreNavRecordingResult> {
  // Get current tab
