...
describe('updateExtensionBadge', () => {
    ...
    describe('badge state priority', () => {
        it('shows placeholders badge even when disconnected, paused, and has active rules', async () => {
            const action = getActionMock();
            await updateExtensionBadge(false, makeActiveRules(5), true, true, null, 10);
        ...
        it('shows disconnected badge when not connected (above threshold), not paused, no placeholders', async () => {
            const action = getActionMock();
            await updateExtensionBadge(false, makeActiveRules(3), false, false, null, 3);
        ...
        it('shows paused badge when paused, connected, no placeholders', async () => {
            const action = getActionMock();
            await updateExtensionBadge(true, makeActiveRules(5), false, true, null, 0);
        ...
        it('shows active badge when connected, not paused, no placeholders, has rules', async () => {
            const action = getActionMock();
            await updateExtensionBadge(true, makeActiveRules(7), false, false, null, 0);
        ...
        it('clears badge when connected, not paused, no placeholders, no rules', async () => {
            const action = getActionMock();
            await updateExtensionBadge(true, [], false, false, null, 0);
    ...
    describe('active rules count display', () => {
        it('shows "1" for a single active rule with singular "rule" in tooltip', async () => {
            const action = getActionMock();
            await updateExtensionBadge(true, makeActiveRules(1), false, false, null, 0);
        ...
        it('shows "50" for 50 rules', async () => {
            const action = getActionMock();
            await updateExtensionBadge(true, makeActiveRules(50), false, false, null, 0);
        ...
        it('shows "99" for 99 rules', async () => {
            const action = getActionMock();
            await updateExtensionBadge(true, makeActiveRules(99), false, false, null, 0);
        ...
        it('shows "99+" for 100 or more rules', async () => {
            const action = getActionMock();
            await updateExtensionBadge(true, makeActiveRules(100), false, false, null, 0);
        ...
        it('shows "99+" for 250 rules', async () => {
            const action = getActionMock();
            await updateExtensionBadge(true, makeActiveRules(250), false, false, null, 0);
    ...
    describe('disconnected badge threshold', () => {
        it('does NOT show disconnected badge when reconnectAttempts < 3', async () => {
            ...
            await updateExtensionBadge(false, [], false, false, null, 2);
        ...
        it('shows disconnected badge when reconnectAttempts === 3', async () => {
            const action = getActionMock();
            await updateExtensionBadge(false, [], false, false, null, 3);
        ...
        it('shows disconnected badge when reconnectAttempts > 3', async () => {
            const action = getActionMock();
            await updateExtensionBadge(false, [], false, false, null, 10);
    ...
    describe('recording active skips badge update', () => {
        it('skips badge update when a tab is recording', async () => {
            ...
            await updateExtensionBadge(true, makeActiveRules(5), false, false, recordingService, 0);
        ...
        it('does not skip badge update when recording service exists but no tab is recording', async () => {
            ...
            await updateExtensionBadge(true, makeActiveRules(3), false, false, recordingService, 0);
    ...
    describe('state deduplication', () => {
        it('does not re-update badge when called twice with same state', async () => {
            ...
            await updateExtensionBadge(true, makeActiveRules(5), false, false, null, 0);
        ...
        it('updates badge when state changes between calls', async () => {
            ...
            await updateExtensionBadge(true, makeActiveRules(10), false, false, null, 0);
